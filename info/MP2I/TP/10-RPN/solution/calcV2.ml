...
let fun_of_string str =
  ...
  | "/" -> ( / )
  | "%" -> ( mod )
  | _ -> raise (Invalid_argument "calculatrice")


(** Réalise le calcul récursif. *)
let rec calcule instr pile =
  match instr with
  | [] -> List.hd pile
  | str :: q ->
      (* Si str est un entier *)
      try calcule q (int_of_string str :: pile)
      (* Sinon c'est une fonction *)
      with Failure _ ->
          let a, b = List.hd pile, List.hd (List.tl pile) in
          let pile_depilee = List.tl (List.tl pile) in
          calcule q ((fun_of_string str a b) :: pile_depilee)
